...
HGLOBAL copyAsciiToHGlobal(const Local<String>& value) {
...
HGLOBAL copyUnicodeToHGlobal(const Local<String>& value) {
    Local<Context> context = Nan::GetCurrentContext();
    Isolate* isolate = context->GetIsolate();
...
HGLOBAL copyCustomToHGlobal(const Local<Value>& value) {
    Local<Context> context = Nan::GetCurrentContext();
    if (value->IsString()) {
        ...
        auto asciiValue = Nan::Get(format, Nan::New("ascii").ToLocalChecked())
                              .ToLocalChecked();
        auto unicode = TRUE;
        if (asciiValue->IsTrue()) {
            unicode = FALSE;
        }
        auto dataValue = Nan::Get(format, Nan::New("data").ToLocalChecked())
                             .ToLocalChecked();
        Local<String> data;
...
LPCSTR getTypeAsString(const Local<Value>& value) {
...
Local<Function> getFunction(const Local<Object>& obj,
                            const std::string& functionName) {
    // TODO check if ToLocalChecked will fail before calling it
    Local<Value> func = obj->Get(Nan::GetCurrentContext(),
...
Local<String> getWindowText(HWND hwnd) {
